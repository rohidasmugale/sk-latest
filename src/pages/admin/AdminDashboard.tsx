...
import {
  ...
  MapPin,
  Plus,
  Coffee, Timer, LogIn, LogOut, Camera
} from 'lucide-react';
...
const fetchRealPayroll = async () => {
 ...
 const handlePayrollFilterChange = () => {
  ...
  navigate(`/admin/attendance?view=site&startDate=${selectedDate}&endDate=${selectedDate}`);
};

  const handleSmallPieChartClick = (dayData: any) => {
    navigate(`/admin/attendance?view=site&startDate=${dayData.date}&endDate=${dayData.date}`);
  };

  const handleDepartmentCardClick = (department: string) => {
    navigate(`/admin/attendance?view=department&department=${department}&startDate=Today&endDate=Today`);
  };
...
<UnifiedCreateModal
  ...
  allowedTabs={['employee', 'client', 'site', 'manager', 'supervisor']}   // ✅ no 'admin'
/>
          ...
          <CardContent className="px-3 sm:px-4 py-3 sm:py-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-4">
              ...
              <div className="flex-1 flex items-end">
                <Button onClick={handlePayrollFilterChange} className="w-full bg-blue-600 hover:bg-blue-700 h-9 text-sm"><Filter className="h-3 w-3 mr-1" />Apply</Button>
              </div>
            </div>
              ...
              {payrollTab === 'list-view' ? (
                <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  <div className="flex flex-col sm:flex-row gap-2 mb-3">
                    ...
                    <Button variant="outline" size="sm" onClick={handleExportToExcel} className="h-8 text-xs"><Download className="h-3 w-3 mr-1" />Export</Button>
                  </div>
                ...
                <motion.div key="chart" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  <div className="mb-3"><label className="text-xs font-medium mb-1 block">Select Site</label><Select value={selectedSite} onValueChange={setSelectedSite}><SelectTrigger className="h-9 text-sm"><SelectValue placeholder="Choose site..." /></SelectTrigger><SelectContent>{payrollData.map(site => <SelectItem key={site.siteName} value={site.siteName}>{site.siteName.split(',')[0]}</SelectItem>)}</SelectContent></Select></div>
                  {selectedSiteData && (
